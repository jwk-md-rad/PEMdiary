...
import { formatDatum, symptoomBgKleur, gemSymptoomScore } from '../utils/helpers.js'
  ...
  return (
    <div className="card hover:border-slate-300 transition-colors">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => onBewerken(entry)}
        ...
        {!bevestigVerwijder ? (
          ...
          <div className="flex gap-1">
            <button onClick={() => onVerwijder(entry.id)}
              className="text-xs text-red-600 bg-red-100 hover:bg-red-200 px-2 py-1.5 rounded-lg font-medium">
              Verwijder
            </button>
            <button onClick={() => setBevestigVerwijder(false)}
              className="text-xs text-slate-500 bg-slate-100 hover:bg-slate-200 px-2 py-1.5 rounded-lg">
              Nee
            </button>
          </div>
        )}
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input type="search" placeholder="Zoek in je dagboek..."
            value={zoek} onChange={e => setZoek(e.target.value)}
            className="input-field pl-9" />
        </div>
        <span className="text-xs text-slate-400 flex-shrink-0">{gefilterd.length} entries</span>
      </div>
      ...
      {gefilterd.length === 0 ? (
        ...
        <div className="space-y-3">
          {gefilterd.map(entry => (
            <EntryRij key={entry.id} entry={entry}
              onDetail={onDetail} onBewerken={onBewerken}
              onVerwijder={deleteEntry} />
          ))}
        </div>
      )}
    </div>
