  ...
  return (
    <div className="card">
      ...
      <div className="flex gap-3 mt-1 justify-center">
        {[['#2563eb', 'Vermoeidheid'], ['#7c3aed', 'Brain fog'], ['#dc2626', 'Pijn']].map(([k, l]) => (
      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {entry.activiteiten?.length ? `${entry.activiteiten.length} activiteit(en)` : 'Geen activiteiten'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {pemStijging !== null && pemStijging > 0 && (
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
              ...
              pemStijging <= 2 ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'
            }`}>PEM +{pemStijging}</span>
          )}
          {gem !== null && (
            <span className={`text-sm font-bold px-2 py-0.5 rounded-lg ${symptoomBgKleur(gem)}`}>∅ {gem}</span>
          )}
        </div>
      </div>
      {entry.symptomen && (
        <div className="flex gap-2 text-xs text-slate-500">
          <span>Moe: <strong className={entry.symptomen.vermoeidheid <= 2 ? 'text-green-600' : entry.symptomen.vermoeidheid <= 5 ? 'text-yellow-600' : 'text-red-600'}>{entry.symptomen.vermoeidheid}</strong></span>
          <span>Fog: <strong>{entry.symptomen.brainFog}</strong></span>
          <span>Pijn: <strong>{entry.symptomen.pijn}</strong></span>
          {entry.nachtrust?.duur && <span className="ml-auto">Slaap: {entry.nachtrust.duur}u</span>}
        </div>
      )}
    </button>
  )
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="card bg-gradient-to-br from-blue-600 to-blue-700 border-0 text-white">
        ...
        <h2 className="text-lg font-bold mb-3">{heeftVandaag ? 'Dag al gelogd' : 'Goede dag!'}</h2>
        ...
        <div>
          ...
          <div className="space-y-2">
            {entries.slice(0, 7).map(entry => (
              <RecenteEntryKaart key={entry.id} entry={entry}
                patroon={patronen.find(p => p.datum === entry.datum)}
                onDetail={onDetail} />
            ))}
          </div>
        </div>
