...
// Chart 1: Symptoom trends over tijd
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mt-2 justify-center">
        {SYMPTOOM_KEYS.map(k => (
          <LegendeItem key={k} kleur={symptoomChartKleur(k)} label={SYMPTOOM_LABELS[k]} />
        ))}
      </div>
    </GrafiekKaart>
  )
...
function PEMPatroonChart({ entries }) {
  ...
  return (
    <GrafiekKaart
      titel="Gemiddeld PEM patroon"
      beschrijving="Hoe symptomen gemiddeld veranderen na activiteit (alle entries)"
    >
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
...
// Chart 3: Activiteitenbelasting vs symptomen
function BelastingChart({ entries }) {
  const data = entries.slice().reverse().map(e => {
    ...
    return {
      ...
      symptomen: gem,
      duur: e.activiteiten?.reduce((s, a) => s + (Number(a.duur) || 0), 0) || 0,
    }
  })

  return (
    <GrafiekKaart
      titel="Activiteitsbelasting vs symptomen"
      beschrijving="Belasting = totale minuten × RPE / 10"
    >
      <ResponsiveContainer width="100%" height={200}>
...
function SlaapChart({ entries }) {
  ...
  return (
    <GrafiekKaart
      titel="Slaap trend"
      beschrijving="Slaapduur en kwaliteit (1=slecht, 2=matig, 3=goed)"
    >
      <ResponsiveContainer width="100%" height={180}>
        <LineChart data={data} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
          ...
          <Tooltip
            ...
            formatter={(val, name) => {
              if (name === 'kwaliteit') {
                const labels = { 1: 'Slecht', 2: 'Matig', 3: 'Goed' }
                return [labels[val] || val, 'Kwaliteit']
              }
              return [val, name === 'duur' ? 'Uren slaap' : 'HRV']
            }}
          />
...
function RadarProfielChart({ entries }) {
  ...
  return (
    <GrafiekKaart
      titel="Gemiddeld symptoom profiel"
      beschrijving="Vergelijking avond vs +24u vs +72u"
    >
      <ResponsiveContainer width="100%" height={220}>
        <RadarChart data={data} margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
          ...
          <Radar name="Avond" dataKey="t0" stroke="#2563eb" fill="#2563eb" fillOpacity={0.2} />
      ...
      <div className="flex gap-3 mt-1 justify-center">
        <LegendeItem kleur="#2563eb" label="Avond" />
...
// Chart 6: RPE spreiding per activiteitstype
function RPEChart({ entries }) {
  const alle = entries.flatMap(e =>
    (e.activiteiten || []).map(a => ({
      datum: formatDatumKort(e.datum),
      rpe: Number(a.rpe) || 0,
      type: a.type || 'fysiek',
      naam: a.activiteit || 'Onbekend',
      duur: Number(a.duur) || 0,
    }))
  ).slice(-50)

  if (alle.length === 0) return null

  const fysiek = alle.filter(a => a.type === 'fysiek')
  const mentaal = alle.filter(a => a.type !== 'fysiek')

  return (
    <GrafiekKaart
      titel="RPE per activiteit"
      beschrijving="Inspanningsperceptie van je recente activiteiten"
    >
      <ResponsiveContainer width="100%" height={180}>
        <ScatterChart margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
          <XAxis dataKey="duur" name="Duur (min)" tick={{ fontSize: 9 }} label={{ value: 'min', position: 'insideRight', fontSize: 9 }} />
          <YAxis dataKey="rpe" domain={[0, 10]} tick={{ fontSize: 9 }} label={{ value: 'RPE', angle: -90, position: 'insideLeft', fontSize: 9 }} />
          <Tooltip
            contentStyle={{ fontSize: 11, borderRadius: 8, border: '1px solid #e2e8f0' }}
            formatter={(val, name) => [val, name]}
            cursor={{ strokeDasharray: '3 3' }}
          />
          <Scatter name="Fysiek" data={fysiek} fill="#2563eb" opacity={0.7} />
          <Scatter name="Mentaal" data={mentaal} fill="#7c3aed" opacity={0.7} />
        </ScatterChart>
      </ResponsiveContainer>
      <div className="flex gap-3 mt-1 justify-center">
        <LegendeItem kleur="#2563eb" label="Fysiek" />
        <LegendeItem kleur="#7c3aed" label="Mentaal/Sociaal" />
      </div>
    </GrafiekKaart>
  )
}
...
export default function Grafieken() {
  ...
  function toggleSymptoom(k) {
    setGeselecteerdeSymptomen(prev =>
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1">
            {[7, 14, 30, 90].map(d => (
              <button
                key={d}
                onClick={() => setGeselecteerdePeriode(d)}
                className={`text-xs px-2.5 py-1 rounded-lg font-medium transition-colors ${
                  geselecteerdePeriode === d
      ...
      <p className="text-xs text-slate-400 text-right">
        {gefilterd.length} entries in geselecteerde periode
      </p>
